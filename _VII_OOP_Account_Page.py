...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def updateData():  # For Update Button
        ...
        if record:
            query3 = """UPDATE main_employee_information_tbl SET Username=?, Password=?, Confirm_Password=?, User_Type=?, User_Status=? WHERE Employee_Number=?"""
            query4 = """UPDATE III_employee_user_account_information_tbl SET Username=?, Password=?, Confirm_Password=?, User_Type=?, User_Status=? WHERE Employee_Number=?"""
            query3_tuple = [Username.get(), Password.get(), Confirm_Password.get(), User_Type.get(), User_Status.get(), employee_number]
            query4_tuple = [Username.get(), Password.get(), Confirm_Password.get(), User_Type.get(), User_Status.get(), employee_number]
